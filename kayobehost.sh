...
for i in {1..1}; do
    virsh detach-interface "node-${i}.cloud.kayobe.dev" network --config

    virsh attach-interface "node-${i}.cloud.kayobe.dev" network virbr-mgt \
        --model virtio --config
    virsh attach-interface "node-${i}.cloud.kayobe.dev" network virbr-ser \
        --model virtio --config

    virsh start "node-${i}.cloud.kayobe.dev"
done
...
172.16.1.11 node-1 node-1.cloud.kayobe.dev
EOF'
